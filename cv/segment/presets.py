...
def get_modules(use_v2):
    ...
    if use_v2:
        import flowvision.transforms.v2
        import flowvision.tv_tensors
        import v2_extras

        return flowvision.transforms.v2, flowvision.tv_tensors, v2_extras
    else:
        import transforms
...
class SegmentationPresetTrain:
    def __init__(
        ...
        *,
        ...
        if use_v2:
            img_type = tv_tensors.Image if backend == "tv_tensor" else flow.Tensor
            transforms += [
                T.ToDtype(dtype={img_type: flow.float32, tv_tensors.Mask: flow.int64, "others": None}, scale=True)
            ]
        else:
