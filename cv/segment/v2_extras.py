...
from flowvision import tv_tensors
from flowvision.transforms import v2
...
class CocoDetectionToVOCSegmentation(v2.Transform):
    ...
    2. COCO only offers detection masks, i.e. a (N, H, W) bool-ish tensor, where the truthy values in each individual
       mask denote the instance. However, a segmentation mask is a (H, W) integer tensor (typically flow.uint8), where
    ...
    def _coco_detection_masks_to_voc_segmentation_mask(self, target):
        ...
        instance_masks = instance_masks[list(valid_voc_category_idcs)].to(flow.uint8)
        instance_labels_voc = flow.tensor(instance_labels_voc, dtype=flow.uint8)
    ...
    def forward(self, image, target):
        ...
        if segmentation_mask is None:
            segmentation_mask = flow.zeros(v2.functional.get_size(image), dtype=flow.uint8)
