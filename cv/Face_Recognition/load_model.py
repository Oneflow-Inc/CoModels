...
def _legacy_tar_load(filename, model_dir, map_location, delete_tar_file=True):
    with tarfile.open(filename) as f:
        ...
        if os.path.isdir(extracted_file):
            ch_dir = os.listdir(extracted_file)
            extracted_file = os.path.join(extracted_file,ch_dir[1])
    if delete_tar_file:
        os.remove(filename)
...
def load_state_dict_from_url(
    ...
    if os.path.exists(pretrained_weight_dir):
        if os.path.isdir(pretrained_weight_dir):
            ch_dir = os.listdir(pretrained_weight_dir)
            pretrained_weight_dir = os.path.join(pretrained_weight_dir,ch_dir[1])
