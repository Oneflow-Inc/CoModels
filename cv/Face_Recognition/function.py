...
class FC7(flow.nn.Module):
    ...
    def forward(self, x, label):
        x = flow.nn.functional.normalize(x, dim=1, eps=1e-10)
        if self.partial_fc:
            (
                mapped_label,
                sampled_label,
            ...
            weight = self.weight
        weight = flow.nn.functional.normalize(
            weight, dim=1, eps=1e-10)
#        weight = x.to_global(placement=flow.placement.all("cuda"),sbp=flow.sbp.broadcast)
#        print(x.placement)
#        print(weight.device)
#        if not x.is_contiguous():
#        x = x.to_consistent(placement=flow.placement.all("cuda"),sbp=flow.sbp.broadcast)
...
class Train_Module(flow.nn.Module):
    ...
    def forward(self, x, labels):
        x = self.backbone(x)
#        if x.is_consistent:
#            x = x.to_global(placement=flow.placement.all("cuda"),sbp=flow.sbp.broadcast)
        x = self.fc(x, labels)
        return x
    ...
    def train_eager(self):
        ...
        for epoch in range(self.start_epoch, self.cfg.num_epoch):
            ...
            for steps in range(one_epoch_steps):
                ...
                features_fc7 = self.margin_softmax(features_fc7[0], label)*64
