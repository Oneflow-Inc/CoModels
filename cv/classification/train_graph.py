...
def main(config):
    ...
    graph_train_model = make_train_graph(model, criterion, optimizer, lr_scheduler)
    graph_eval_model = make_eval_graph(model)

    model.eval()
    if config.MODEL.RESUME:
        acc1, acc5, loss = validate(config, data_loader_val, graph_eval_model)
        logger.info(
    ...
    if config.THROUGHPUT_MODE:
        throughput(data_loader_val, graph_eval_model, logger)
        return
...
def train_one_epoch(
    config, model, data_loader, epoch, mixup_fn, optimizer
):
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        one_sample_time.record(samples.size(0))
        loss_meter.record(loss.to_local().clone().cpu().detach(), targets.size(0))
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            etas =  (num_steps - idx) * samples.size(0) / throughput_avg
            one_sample_time.reset()

            logger.info(
                f"Train: [{epoch}/{config.TRAIN.EPOCHS}][{idx}/{num_steps}]\t"
                f"eta {datetime.timedelta(seconds=int(etas))}\tlr {lr:.6f}\t"
                f"time {samples.size(0) / throughput:.4f}s ({samples.size(0) / throughput_avg:.4f}s)\t"
...
def validate(config, data_loader, model):
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        batch_time.record(target.size(0))
